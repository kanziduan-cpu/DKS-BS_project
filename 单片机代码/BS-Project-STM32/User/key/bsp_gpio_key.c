    ...
    switch(key_info->event)
    {
        ...
        case EVENT_PRESS:       
            ...
            if(key_info->status == KEY_DOWN)
            {
                ...
                {
                    key_info->event = EVENT_LONG;           //杩涘叆 闀挎寜 浜嬩欢
                } 
            }
            else
            {
                key_info->event = EVENT_SHORT;              //杩涘叆 鐭寜 浜嬩欢 
            }
            break;
            
        case EVENT_SHORT:
            key_info->event = EVENT_SHORT_RELEASE;          //涓婁竴杞繑鍥炰簡EVENT_SHORT锛屾棤杩涢樁浜嬩欢锛岄噸鍥濫VENT_ATTONITY
//            key_info->clicktype = KEY_SINGLE_CLICK;
        ...
        case EVENT_LONG:
            if(key_info->status == KEY_DOWN)
            {
                key_info->event = EVENT_LONG;           //淇濇寔 闀挎寜 浜嬩欢
            }
            else
            {
                key_info->event = EVENT_LONG_RELEASE;      //涓婁竴杞繑鍥炰簡EVENT_LONG锛屾棤杩涢樁浜嬩欢锛岄噸鍥濫VENT_ATTONITY
