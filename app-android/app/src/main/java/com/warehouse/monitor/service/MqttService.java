...
public class MqttService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        try {
            Notification notification = createNotification("Monitoring service is running...");
    ...
    private void setupMqttListeners() {
        connectionStatusListener = (status, message) -> {
            ...
            Log.d(TAG, "Connection status: " + status + " - " + message);
        };
        mqttManager.addConnectionStatusListener(connectionStatusListener);

        environmentDataListener = data ->
                Log.d(TAG, "Environment data received: temp=" + data.getTemperature() + ", humidity=" + data.getHumidity());
        mqttManager.addEnvironmentDataListener(environmentDataListener);

        deviceStatusListener = (deviceId, isOnline, isRunning) ->
                Log.d(TAG, "Device status updated: " + deviceId + " - online=" + isOnline + ", running=" + isRunning);
    ...
    private void handleAlarm(Alarm alarm) {
        if (alarm == null) {
            Log.w(TAG, "Received empty alarm payload, ignoring.");
            return;
        }
        ...
        if (TextUtils.isEmpty(alarm.getAlarmTitle())) {
            alarm.setAlarmTitle("Warehouse alarm");
        }
        if (TextUtils.isEmpty(alarm.getAlarmMessage())) {
            alarm.setAlarmMessage("A device reported an abnormal condition. Please inspect it.");
        }
    ...
    private Notification createNotification(String status) {
        ...
        return new NotificationCompat.Builder(this, WarehouseMonitorApp.CHANNEL_ID_DATA)
            ...
            .setContentTitle("Warehouse Monitor")
    ...
    private String getStatusText(MqttManager.ConnectionStatus status, String message) {
        switch (status) {
            case CONNECTED: return "Connected to broker";
            case CONNECTING: return "Connecting to broker...";
            case DISCONNECTED: return "Disconnected";
            case ERROR: return "Connection error: " + message;
